KiddoPaint.Tools.Toolbox.Fuzzer = function () {
  ...
  this.toolDraw = function () {
    var target = KiddoPaint.Display.main_context.getImageData(
      tool.currentEv._x - tool.size(),
      tool.currentEv._y - tool.size(),
      2 * tool.size(),
      2 * tool.size(),
    );
    var ctx = tool.isDown
    ...
    const jitterx = getRandomFloat(-7, 7);
    const jittery = getRandomFloat(-7, 7);
    ...
    ctx.putImageData(
      target,
      tool.currentEv._x - tool.size() + jitterx,
      tool.currentEv._y - tool.size() + jittery,
    );
  };
};

...
function scaleImageDataPixelwise(imageData, scale) {
  var scaled = KiddoPaint.Display.main_context.createImageData(
  ...
  for (var row = 0; row < imageData.height; row++) {
    for (var col = 0; col < imageData.width; col++) {
      var sourcePixel =
        imageData.data[(row * imageData.width + col) * 4 + 0] == 0 &&
        imageData.data[(row * imageData.width + col) * 4 + 1] == 0 &&
        imageData.data[(row * imageData.width + col) * 4 + 2] == 0 &&
        imageData.data[(row * imageData.width + col) * 4 + 3] == 0
          ? [255, 255, 255, 255]
          : [
...
function greyscaleImageData(imageData) {
  var grey = KiddoPaint.Display.main_context.createImageData(
    imageData.width,
    imageData.height,
  );
...
function ditherImageData(imageData) {
  ...
  var dithered = KiddoPaint.Display.main_context.createImageData(
    imageData.width,
    imageData.height,
  );
  ...
  for (var pixel = 0; pixel <= imageDataLength; pixel += 4) {
    ...
    var map = Math.floor(
      (imageData.data[pixel] + bayerThresholdMap[x % 4][y % 4]) / 2,
    );
    dithered.data[pixel] =
      dithered.data[pixel + 1] =
      dithered.data[pixel + 2] =
        map < threshold ? 0 : 255;
